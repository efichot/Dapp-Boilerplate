...
<Flex {...props}>
  <Box flex={['1 1 0']} px={[10]} textAlign="center" {...props.iconWrap} >
    <Link to={`/dashboard/users/${props.uid}/${props.destination}`}>
      {!props.svg ? null : <SVG svg={props.svg}  {...props.icon} w={[30]} h={[30]} /> }
    </Link>
    </Box>
  <Box flex={['4 1 0']} pl={[10]} py={[10]} {...props.titleWrap}>
    <Link to={`/dashboard/users/${props.uid}/${props.destination}`} color='charcoal' >
