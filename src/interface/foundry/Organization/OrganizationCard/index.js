...
export default (props) => {
  ...
  const{ id, data } = props
  /*--- Extraction ---*/
  const nameOrg = idx(props.data, _ => _.name.nameOrganization)
  const nameLegal = idx(props.data, _ => _.name.nameOrganizationLegal)
  ...
  return <Flex direction={['row']} align='stretch' justify='center' p={[10]} {...props} key={id} >
      <Box {...mainLayout}>
        <Link to={`/dashboard/organization/${id || null }`} color='blue' >
          <Heading f={[4,4,5]} level={3} display='inline-block' children={nameOrg|| null} />
          <Heading f={[2,3]} level={3} color='blue' ml={[5]} display='inline-block' children={nameLegal} />
        </Link>
